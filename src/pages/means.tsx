...
/**
 * using ssr static routes with query
 */
...
const App = ({serverData}: any) => {
    ...
    return (
        <div className="app">
            ...
            <small>
                {serverData.word && `${serverData.word} means: ${meanings?.length > 0 ? '' : `invalid word`}`}
            </small>
            <ul>
...
export async function getServerData(context: any) {
    let param = context?.query?.word;
    if (!param) {
        return {
            status: 200, // The HTTP status code that should be returned
            props: {word: ''}, // Will be passed to the page component as "serverData" prop
            headers: {},
        };
    }
